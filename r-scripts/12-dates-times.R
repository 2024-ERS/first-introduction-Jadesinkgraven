...
dat_month |> ggplot(aes(x=mdate,y=avgtemp_mo_oC)) +
  ...
  ggtitle("Monthly average temperature at Lauwersoog (2014-2024)") +
  theme(text = element_text(size=12))


# plot monthly rainfall with 1-year running average
# and fix the text of the axes
dat_month %>% ggplot(aes(x=mdate,y=totrain_mo_mm)) +
  geom_line() +
  tidyquant::geom_ma(ma_fun=SMA, n=13, col="turquoise", linetype="solid")+
  coord_x_date(xlim=c("2014-01-01", "2024-08-31")) +
  ggtitle("Monthly total rainfall (mm) at Lauwersoog (2014-2024)") +
