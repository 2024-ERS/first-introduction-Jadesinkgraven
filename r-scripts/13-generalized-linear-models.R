...
m1 <- lmerTest::lmer(Value ~ Treatment + (1|Block), data = data)
summary(m1)   # show the model summary
coef(m1)      # slope of treatment is the same for every effect
ggplot(data, aes(x = Treatment, y = Value, color = Block)) +
  geom_jitter(width=0.15) +
 geom_line(aes(y=predict(m1)), size=1)

#using lme4, show a mixed model with fixed slopes (=effect of the treatment within each block) and random intercepts
m2 <- lmerTest::lmer(Value ~ Treatment + (Treatment|Block), data = data)
summary(m2)   # show the model summary
coef(m2)      # slope of treatment is the same for every effect
ggplot(data, aes(x = Treatment, y = Value, color = Block)) +
  geom_jitter(width=0.15) +
  geom_line(aes(y=predict(m2)), size=1)

#compare the models using Akaikes Information Criterion (AIC)
#if at least 2 values are lower, then often significantly lower
AIC(m1, m2)
