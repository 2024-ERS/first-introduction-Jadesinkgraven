...
# I can add letters for and for C V R they are a b b
# means with same levels are not significantly different
...
# is this better?
AIC(m4, m3)
# not teally, a normal distribution is also ok
...
anova(m8)
AIC(m8, m7)
# yes it matters, the difference between treatments depends on the block
...
# first fit a model with only block
m9 <- glmer(specrich ~ (1|block), data = dat, family=poisson(link="log"))

# add treatment to the model, assuming fixed slopes (i.e. treatment effect is the same for every block)
m10 <- glmer(specrich ~ graztreat + (1|block), data = dat, family=poisson(link="log"))
anova(m10, m9)
...
m11 <- glmer(specrich ~ graztreat + (graztreat|block), data = dat, family=poisson(link="log"))
anova(m11, m10)
