...
vegan::orditorp(nmds_veg, dis="sp", priority = SpecTotCov, col="red", pcol="red", pch="+", cex=1.1)
...
# so only 8 species go through their optimum
...
vegan::ordiplot(dca,display="sites", cex=0.7, type="t" )
...
ef_dca <- vegan::envfit(dca~elevation_m+ clay_cm+ floodprob+ DistGulley_m+ redox5+ 
                          redox10,data= envdat, na.rm=T)
...
vegan::ordisurf(dca, envdat$clay_cm, add=T, col="green", label=T)
vegan::ordisurf(dca, envdat$elevation_m, add=T, col="brown", label=T)

# add a spe
vegan::ordisurf(dca, vegdat$FestuRub, add=T, col="pink", label=T)

##### make the same plot but using a nmds
##### fit the environmental factors to the nmds ordination surface

ef_nmds <- vegan::envfit(nmds_veg~elevation_m+ clay_cm+ floodprob+ DistGulley_m+ redox5+ redox10,data= envdat, na.rm=T)
ef_nmds
...
anova(cca, by="axis")
anova(cca, by="margin")
#only floodprob is significant
#redo with only the most important uncorrelated variables
cca2 <- vegan::cca(vegdat~floodprob+ DistGulley_m ,data= envdat)
summary(cca)

# add the environmental factors to the cca ordination plot
vegan::ordiplot(cca2, display="sites", type="t", cex=1,
                xlab="CCA1 (21%)", ylab="CCA2 (14%)")
vegan::orditorp(cca2, display="species", priority=SpecTotCov, 
                 col="red", pcol="red", pch="+", cex=0.8)

vegan::ordisurf(cca2, envdat$floodprob, add=T, col="blue", label=T)
vegan::ordisurf(cca2, envdat$DistGulley_m, add=T, col="green", label=T)

# for example - test this if you would have only measured clay thickness
cca3 <- vegan::cca(vegdat~clay_cm,data= envdat)
summary(cca3)
...
vegan::ordiplot(cca1, display="sites", type="t", cex=1,
                 xlab="CCA1 (21%)", ylab="CCA2 (14%)")
